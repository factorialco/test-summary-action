...
exports.generateSummary = exports.createTestsData = exports.generateCypressTestSummary = void 0;
const core = __importStar(__nccwpck_require__(186));
const s3_1 = __nccwpck_require__(863);
const generateCypressTestSummary = (reportData, s3Config) => {
    const data = (0, exports.createTestsData)(reportData, s3Config);
    return (0, exports.generateSummary)(data);
};
exports.generateCypressTestSummary = generateCypressTestSummary;
const createTestsData = (reportData, s3Config) => {
    const data = reportData.results
        .flatMap(({ fullFile, suites }) => suites.flatMap(({ title: suiteTitle, tests }) => tests.map(({ title, fail, err: { message } }) => fail
        ? { file: fullFile, suite: suiteTitle, test: title, error: message }
        : null)))
        .filter((testData) => testData !== null);
    const screenshotUrls = (0, s3_1.getCypressScreenshotUrls)(data, s3Config);
    return data.map(testData => (Object.assign(Object.assign({}, testData), { screenshotUrl: screenshotUrls[testData.file] })));
};
...
function run() {
    try {
        ...
        const reportFile = core.getInput('reportFile');
        const s3Config = {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.getCypressScreenshotUrls = void 0;
const core = __importStar(__nccwpck_require__(186));
const getCypressScreenshotUrls = (data, { bucketName, region }) => {
    ...
    const screenshotEntries = data.map(({ file, test }) => {
        return [
            file,
            encodeURI(`${baseUrl}/${objectPrefix}${file.replace(/cypress\/integration/, '')}/${test}.png`)
        ];
    });
    return Object.fromEntries(screenshotEntries);
};
exports.getCypressScreenshotUrls = getCypressScreenshotUrls;
